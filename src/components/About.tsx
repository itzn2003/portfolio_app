        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <div className="mt-10 flex flex-wrap gap-4">
              {aboutContent.infoCards.map((card, index) => (
                <div key={index} className="px-6 py-3 bg-white border border-gray-200 shadow-sm">
                  <p className="text-sm text-gray-500 font-medium">{card.label}</p>
                  <p className="text-base font-mono text-black">{card.value}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="aspect-square bg-gradient-to-br from-gray-100 to-gray-200 rounded-lg overflow-hidden shadow-card border-4 border border-[#8b00ff] clipped-corners">
              ...
              <div className="w-full h-full flex items-center justify-center">
                <div className="text-center">
                  <img src={aboutContent.image} alt={aboutContent.imageAlt} />
                </div>
              </div>
            </div>
            {/* Accent element */}
